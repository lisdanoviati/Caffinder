...
private function extractDistrict($address)
{
    ...
    // Format: "Kec. XXXX"
    if (preg_match('/Kec\. ([^,]+)/i', $address, $m)) {
        $district = "Medan " . trim($m[1]);
    }
    // Format: "Medan XXXXX"
    else if (preg_match('/Medan\s+([A-Za-z\.]+)/i', $address, $m)) {
        $district = "Medan " . str_replace('.', '', trim($m[1]));
    }
    ...
    // NORMALISASI: gabung variasi jadi satu
    $district = str_ireplace(
        [
            'Medan Bar.', 'Medan Baru.',
            'Medan Tim.'
        ],
        [
            'Medan Baru', 'Medan Baru',
            'Medan Timur'
        ],
        $district
    );
    ...
    public function getDistricts()
    {
        $query = "
            ...
            WHERE {
                ...
                # Jika ada 'Kec. Xxxxx' → ambil Xxxxx saja (tanpa 'Medan')
                BIND(
                    IF(
                        REGEX(?alamat, \"Kec\\\\.\"),
                        REPLACE(?alamat, \".*Kec\\\\. ([^,]+).*\", \"$1\"),
                ...
                # Jika tidak ada 'Kec.' → fallback cari 'Medan Yyyyy'
                BIND(
                    IF(
                        ?kecFromKec != \"\",
                        ?kecFromKec,
