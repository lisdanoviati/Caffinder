...
<!-- NAVBAR -->
<div 
    class="navbar fixed top-0 left-0 w-full flex justify-between items-center px-6 py-3 shadow z-50"
    style="background:#8B5E3C;"
>
    <div class="flex items-center gap-3">
        <img src="/images/logo.png" alt="Logo" class="h-9 w-9 object-cover">
        <span class="text-2xl font-bold text-white drop-shadow-md">Caffinder</span>
    </div>
...
<!-- SEARCH FORM -->
<form action="/search" method="GET" class="mt-8 mb-10 flex items-center gap-3">
    ...
    <input 
        ...
        class="flex-[3] border rounded-full px-5 py-2.5 transition bg-[#F9F5F0] border-gray-300
               text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#8B5E3C]
               dark:bg-[#1f1b18] dark:border-[#8B5E3C] dark:text-white dark:placeholder-gray-300"
    >
    ...
    <div class="relative">
        <div 
            ...
            class="border border-gray-300 rounded-full px-4 py-2.5 cursor-pointer w-56 bg-[#F9F5F0]
                   hover:border-[#8B5E3C] transition flex items-center justify-between text-gray-700
        ...
        >
            ...
            <select 
                name="category" 
                class="w-full border rounded-lg px-3 py-2 mb-3 bg-white 
                       dark:bg-[#1f1b18] dark:border-[#8B5E3C] dark:text-gray-200"
            >
                <option value="">Semua</option>
                @foreach($categories as $cat)
                    <option value="{{ $cat['id'] }}" @selected($cat['id'] == ($filters['category'] ?? ''))>
                        {{ $cat['name'] }}
                    </option>
                @endforeach
            </select>
    ...
    <select 
        name="district" 
        class="flex-[1] border rounded-full px-4 py-2.5 transition bg-[#F9F5F0] text-gray-700 
               border-gray-300 focus:ring-2 focus:ring-[#8B5E3C] focus:outline-none
    ...
    <!-- BUTTON -->
    <button 
        class="px-6 py-2.5 text-white rounded-full font-medium transition"
        style="background:#8B5E3C;"
    >
        Cari
    </button>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    >
        ...
        <div class="p-4">
            ...
            <p class="mt-2 text-[#8B5E3C] font-semibold">
                ⭐ {{ $cafe['rating'] ?? '0' }}
            </p>
        </div>
    </a>
